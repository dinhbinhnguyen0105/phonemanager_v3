...
class DeviceController(BaseController[Device]):
    ...
    def launch_scrcpy(self, devices: List[Device]):
        ...
        for index, device in enumerate(devices):
            if device.device_id in self._active_scrcpy_clients:
                logger.warning(f"Scrcpy for device {device.device_id} is already running. Skipping...")
                continue
            ...
            client.finished.connect(self._on_scrcpy_finished, Qt.ConnectionType.QueuedConnection)
    ...
    def launch_scrcpy_for_job(self, device: Device, max_concurrent_jobs: int):
        ...
        self._active_scrcpy_clients[device.device_id] = client
        client.start_mirroring(x=pos_x, y=pos_y, width=window_width, height=window_height)
        time.sleep(.5)
    ...
    def enable_redsocks(self, devices: List[Device], ip: str, port: int, username: str = "", password: str = "", ptype: str = "http-connect"):
        """
        ...
        for device in devices:
            ...
            if task_name in self._active_workers:
                logger.warning(f"Redsocks for device {device.device_id} is already running. Skipping...")
                continue
            driver = RedsocksDriver(device.device_id)
            worker = GenericWorker(
                task_name=task_name,
                target_func=driver.enable,
                ip=ip, port=port, username=username, password=password, ptype=ptype
            )
    ...
    def _on_redsocks_enable_success(self, task_name: str, result: str):
        """Processes successful proxy activation and updates device state."""
        if task_name not in self._active_workers: return
    ...
    def _on_redsocks_disable_success(self, task_name: str, result: Any):
        """Processes successful proxy deactivation."""
        if task_name not in self._active_workers: return
        device = self._active_workers[task_name]["device"]
        del self._active_workers[task_name]
    ...
    def _on_proxy_error(self, task_name: str, error_msg: str):
        """Handles exceptions encountered during proxy configuration tasks."""
        if task_name not in self._active_workers: return
        device = self._active_workers[task_name]["device"]
        action = self._active_workers[task_name]["action"]
        del self._active_workers[task_name]
        
        logger.error(f"[{device.device_name}] Error running proxy {action} task: {error_msg}")
        if action == "enable":
